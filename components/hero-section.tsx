      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="flex min-h-[80svh] flex-col items-center justify-center py-24 text-center">
          ...
          >
            ...
            <p className="mt-5 max-w-2xl text-pretty text-lg leading-relaxed text-muted-foreground sm:text-xl md:text-2xl">
              {site.subtitle}
            </p>
            ...
            <div className="mt-9 flex w-full max-w-lg flex-col gap-3 sm:max-w-none sm:flex-row sm:justify-center">
              {heroButtons.map((button) => {
                const isPrimary = button.kind === "primary"
                const className = isPrimary
                  ? "inline-flex items-center justify-center gap-2 rounded-full bg-primary px-7 py-3.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/10 transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background sm:text-base"
                  : "inline-flex items-center justify-center gap-2 rounded-full border border-border bg-background/70 px-7 py-3.5 text-sm font-semibold text-foreground backdrop-blur-sm transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background sm:text-base"
                ...
                return (
                  <a key={button.label} href={button.href} className={className}>
                    {button.label}
                    {isPrimary ? <ArrowDown className="h-4 w-4" aria-hidden /> : null}
                  </a>
                )
              })}
            </div>
