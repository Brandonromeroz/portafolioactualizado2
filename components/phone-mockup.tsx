...
export function PhoneMockup({ label, src, alt, className, offset = "center" }: PhoneMockupProps) {
  return (
    <figure
      className={cn(
        "relative aspect-[9/19.5] w-full max-w-[12.25rem] overflow-hidden rounded-[0.95rem] bg-zinc-950 shadow-[0_18px_40px_-16px_rgba(0,0,0,0.7)] ring-1 ring-white/15 sm:rounded-[1.35rem]",
        offset === "left" && "lg:-rotate-6 lg:translate-y-4",
        offset === "right" && "lg:rotate-6 lg:translate-y-4",
        offset === "center" && "lg:z-10 lg:-translate-y-2",
        className,
      )}
    >
      {src ? (
        <Image src={src} alt={alt ?? label} fill className="object-cover" sizes="(min-width: 768px) 196px, 30vw" />
      ) : (
        <div className="flex h-full flex-col items-center justify-center gap-2 border border-dashed border-zinc-500/70 bg-zinc-800/90 px-2 text-center sm:px-4">
          <Smartphone className="h-5 w-5 text-zinc-300 sm:h-6 sm:w-6" aria-hidden />
          <figcaption className="text-[10px] leading-snug text-zinc-200 sm:text-[11px]">
            <span className="mb-1 block text-[9px] font-semibold tracking-[0.16em] text-zinc-300 uppercase sm:text-[10px]">
              Captura
            </span>
            {label}
          </figcaption>
...
export function PhoneMockupRow({ slots, screenshots = [], title }: PhoneMockupRowProps) {
  ...
  return (
    <div
      className="mx-auto grid w-full grid-cols-3 items-end justify-items-center gap-1.5 sm:gap-3 md:gap-4 lg:px-3 lg:py-5"
      role="group"
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-zinc-200/80 bg-zinc-50 px-3 py-2 sm:px-4 sm:py-2.5 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-zinc-300 dark:bg-zinc-600" aria-hidden />
        <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-zinc-300 dark:bg-zinc-600" aria-hidden />
        <span className="ml-2 min-w-0 truncate text-xs text-muted-foreground">{title}</span>
      </div>
      <div className="relative aspect-[16/10] min-h-[12rem] w-full bg-gradient-to-br from-sky-50 via-white to-fuchsia-50 sm:min-h-[16rem] dark:from-zinc-900 dark:via-zinc-950 dark:to-indigo-950">
        {logo ? (
          <div className="absolute top-3 left-3 z-10 w-20 sm:top-5 sm:left-5 sm:w-36">
